...
files, cut short if it is longer than a response can carry. The tools that
diff one version against another arrive with
...
`list_package_files`, which fetches that archive and lists the paths inside it
with the top-level directory stripped; and `get_file_content`, which returns
one of those files, saying when it had to cut one short and when the bytes
were not valid UTF-8. The tools that diff one version against another arrive
with
[#13](https://github.com/philfreshman/diffpack-server/issues/13) onward.
